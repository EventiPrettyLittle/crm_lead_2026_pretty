...
import { User, LogOut, ChevronRight, LayoutGrid, ShieldCheck, Loader2 } from "lucide-react"
...
export function AppSidebar() {
    ...
    useEffect(() => {
        // Caricamento impostazioni e utente in parallelo
        Promise.all([
                        ...
                        <SidebarMenu className="gap-1.5">
                            {loading ? (
                                <div className="px-4 py-2 space-y-4">
                                    ...
                                    <div className="h-10 bg-slate-100 animate-pulse rounded-xl" />
                                    <div className="h-10 bg-slate-100 animate-pulse rounded-xl opacity-50" />
                                </div>
                            ) : (
                        ...
                        <Link href="/settings" className="flex flex-1 items-center gap-3 hover:opacity-80 transition-opacity min-w-0">
                            ...
                            <div className="flex-1 min-w-0 text-left">
                                <p className="text-xs font-bold leading-none truncate">
                                    {loading ? "..." : (user?.name || (user?.email ? user.email.split('@')[0] : "Account"))}
                                </p>
                                <p className="text-[10px] opacity-60 font-medium truncate mt-0.5">
                                    {loading ? "In attesa..." : (user?.role || "Utente")}
                                </p>
                            </div>
                        </Link>
                        <button 
                            onClick={handleLogout}
                            className="p-2 rounded-xl hover:bg-white/10 transition-all text-slate-400 hover:text-rose-400"
                        >
